...
| `approve_aesthetic_proxy` | `offline.hyperframes-720p-aesthetic-proxy.v2` | Under workflow v3, render and review one full-length HyperFrames 1280×720 A/A+B proxy with frozen narration under `aesthetic_proxy_approvals_per_batch=1`; opening/middle/ending images are navigation evidence. Bind to `aesthetic_proxy_approval_integrity` plus `workflow_v3_release_integrity`, and freeze structure, UID, framing, geometry, typography, motion and pace before target render. Legacy v2 retains its excerpt contract under the same action key. |
...
That order is mandatory for `hyperframes_proxy_gated_v2`: source-SHA/profile-hash proxies in a local non-iCloud cache (default 1080p/CFR30/H.264/yuv420p/BT.709/GOP≤30) → proxy-bound index → 4–8 second semantic-unit match and review on the canonical SRT cue clock → real 30–60 second HyperFrames stress render → workflow-v3 full-length HyperFrames 720p A/A+B proxy with frozen narration and artifact-bound approval (legacy v2 keeps its frozen excerpt gate) → one target-resolution master → one complete master QA → chunk-scoped repairs only. A normal semantic unit retains 8–12 candidates; a risk unit may expand to no more than 32 and must present at least three ranked A/B/C alternatives. The aesthetic approval freezes structure, UID, framing, geometry, typography, motion, and pace; changing a frozen decision requires returning through the appropriate repair/review and approval gates rather than silently altering the master.
