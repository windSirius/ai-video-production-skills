...
visuals/
  ...
  candidate_review/
    ...
    risk/
  review_manifest.json
  static_asset_review_bundle.json
  semantic_coverage_qa.json
  reuse_qa.json
  aesthetic_review/
    approval.json
    opening/
...
- `workflow_profiles.production_control=artifact_bound_release_v3` for new runs, with artifact paths for approval ledger, research matrix, script QA, voice release, semantic coverage, visual-family reuse, full review proxy/approval, cover review and `CURRENT.json`;
- v3 match sheet, A visual review manifest, B review manifest, static-asset review bundle, HyperFrames composition/render plan/stress manifest and their current paths;
...
## Workflow v3 release minimum

Follow [workflow-v3-production-contract.md](workflow-v3-production-contract.md). Before target render require:

- `research/evidence_matrix.tsv` with reviewed core claims, counterevidence and confidence;
- `script/script_qa.json` with whole-document structure/evidence/Chinese/persona/pronoun/read-aloud passes bound to the script SHA;
- `narration/voice_release.json` with lexical, P1 pronunciation, prosody, punctuation-topology and full-length-audition passes bound to the actual narration SHA;
- `visuals/semantic_coverage_qa.json` with A direct+strong≥0.80, A+B≥0.90, unresolved=0 and CG/PV-like 0–10 second hook;
- `visuals/reuse_qa.json` with complete visual-family IDs, maximum same-family streak≤3, no unresolved overlap and no unapproved cross-episode cooldown violation;
- `script_audio_freeze` bound to the current authority bundle and `static_asset_review` bound to `visuals/static_asset_review_bundle.json`, whose hashes in turn bind the current A/B review manifests;
- one full-length 1280×720 proxy covering `[0,target_frame_count)`, including frozen narration and binding match/A/B/static bundle/composition/render-plan/stress hashes, plus an artifact-bound `full_proxy_render_authorization` written after the proxy was generated and shown;
- `scripts/audit_workflow_v3.py RUN_DIR --stage pre-render` PASS.

At delivery, BGM requires a `bgm_mix_review` bound to the audition mix the user actually heard, and the cover requires a `cover_review` bound to the 16:9 file the user actually saw. Root `CURRENT.json` must bind one current script, narration, subtitle, picture master, BGM master and approved 16:9 cover by path/SHA; those BGM/cover hashes must match their review manifests. Historical PASS reports remain recoverable but are marked `superseded`; they do not compete with `CURRENT.json`.
...
- under workflow v3, the HyperFrames 1280×720 aesthetic proxy is a full-length continuous A or A+B review containing frozen narration, current composition/render-plan/passing-stress/semantic-coverage/reuse hashes, complete-frame coverage, opening/middle/ending evidence and frozen decisions for structure, UID, framing, geometry, typography, motion and pace; v2 legacy runs retain their frozen excerpt proxy contract;
...
The manifest also binds the current authority-bundle revision, final narration SHA, final SRT SHA, fps and target frame count. An audition mix must use that exact narration file; equal duration or tail trimming is not a valid binding. Workflow v3 additionally records `bgm_master_path`, `bgm_master_sha256`, `audition_mix_path`, `audition_mix_sha256` and an `approval_ledger_id` pointing to `bgm_mix_review`.
...
Record the one-sentence episode-specific cover promise, then one `cover_sources.json` row per official/user/generated asset with distinct `identity_model`, `shot_camera`, `style_render` or `typography` role. Bind each output to its source hashes, local layout source, dimensions and thumbnail preview. Named characters, official weapons, UI and icons default to `exact_subject_route=deterministic_official_foreground`; generated full scenes require explicit user authorization and may not silently redraw official weapon/UI geometry. Require a passing 16:9 no-text identity/thesis/same-space review before typography or other ratios, followed by thumbnail and edge-artifact checks; each ratio then receives its own human verdict. The final review manifest records the approved 16:9 artifact path/SHA and a `cover_review` ledger ID; post-approval edits invalidate it. A previous episode's composition is style evidence only unless the current promise independently justifies the same shot.
...
- the mandatory order: local source-SHA/profile-hash proxies → proxy-bound index → 4–8 second semantic-unit matching/review with normal pools of 8–12 and risk pools capped at 32 with at least A/B/C → real 30–60 second HyperFrames stress render → workflow-v3 full-length 720p A/A+B approval with frozen narration (or a frozen legacy-v2 excerpt contract) → one target-resolution master → one full QA → chunk-scoped patch;
...
- HyperFrames workflow-v3 full-length 720p proxy evidence plus opening/middle/ending navigation evidence, the user's separate artifact-bound approval and the frozen structure, UID, framing, geometry, typography, motion and pace decisions;
