...
5. **Obtain user approval on 720p aesthetic proxies.** Render the opening, ending, and at least one representative middle interval. Confirm hook, evidence density, continuity, shot cadence, emotional release and final-image function. A producer or machine-authored PASS is not user approval.
...
| Aesthetic proxy gate | passing stress gate and current composition | 720p opening, ending, and representative-middle proxies with separate objective and user-review verdicts | objective checks pass and the user explicitly approves hook, continuity, cadence, evidence, emotional release and final-image function |
...
- Picture generation: one current-generation HyperFrames composition/render plan, one 30–60 second stress sample, one 720p opening/ending/middle proxy bundle, one explicit user creative verdict, one planned target-resolution master, one final-QA report, or one declared incremental patch per batch. Picture application is one live replacement and never includes caption mutation.
