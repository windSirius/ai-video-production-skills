...
3. After the stress gate passes, workflow v3 renders one full-length continuous 720p A/A+B proxy with frozen narration. Opening, ending, and representative-middle excerpts remain navigation evidence. Obtain artifact-bound approval for the displayed proxy SHA; repair and rerender the full proxy lineage when any bound visual/composition input changes. Legacy v2 may retain its frozen excerpt contract.
4. Only after the frozen authority chain, whole-plan reuse/black audit, technical stress gate and explicit user proxy verdict all pass, perform one planned full-length HyperFrames production render at the contract target resolution—2560×1440/60fps for Alan's current default—from the current reviewed match generation. Match the live-derived narration timing contract within one frame and include exactly one video stream with no audio stream. Do not use a full-resolution render as the aesthetic-review proxy.
